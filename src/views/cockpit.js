...
import { esc, money, mount, ring, bar, chip, stat, empty, rel, fmtDT, toast, modal, pct } from '../ui.js';
...
export async function render(el) {
  ...
  const draw = (d) => `
    <div class="page-head">
      ...
      <button class="btn amber sm" data-quick>+ Liên hệ mới</button>
    </div>
    ...
    <div class="card">
      ...
      <div class="row mt sm mut">📌 Định mức realtime — mọi hoạt động ghi 1 lần, tự tính vào KPI & báo cáo.</div>
    </div>

    <div class="grid g3 mt">
      ${stat('KPI tháng', d.kpi.total + '<span class="sm mut">/100</span>', 'Xếp loại ' + d.kpi.grade, 'red')}
      ${stat('Deal quá SLA', d.riskyCount, d.riskyCount ? 'Cần chăm ngay' : 'Sạch SLA 👍', d.riskyCount ? 'amber' : '')}
      ${stat('Việc hôm nay', d.taskCount, d.reportSubmitted ? 'Đã nộp EOD ✅' : 'Chưa nộp EOD', 'blue')}
    </div>

    <div class="sec-title">Nhắc thông minh</div>
    <div class="card">${d.reminders.map(r => `<div class="item">
      <div class="dot-i">${r.level === 'danger' ? '🚨' : r.level === 'warn' ? '⚠️' : r.level === 'ok' ? '✅' : 'ℹ️'}</div>
    ...
    <div class="card">${d.tasks.length ? d.tasks.map(t => `<div class="item">
      ...
      </div>`).join('') : empty('🎯', 'Không còn việc tồn — chủ động tìm khách mới!')}</div>

    <div class="sec-title">Deal cần chăm gấp (SLA)</div>
    <div class="card">${d.risky.length ? d.risky.map(x => `<div class="item">
        <div class="dot-i">🔥</div>
      ...
      </div>`).join('') : empty('✅', 'Không có deal nào vượt SLA.')}</div>

    <div class="sec-title">Thông báo gần đây</div>
    <div class="card">${d.notifications.length ? d.notifications.map(n => `<div class="item">
      ...
      </div>`).join('') : empty('🔔', 'Chưa có thông báo.')}</div>`;
